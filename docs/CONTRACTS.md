...
CI and Security concurrency belongs to the thin caller so both lanes can coexist while a newer run of the same caller/ref cancels obsolete work. Reusable Release publication owns a version-scoped non-cancelling concurrency lock; release metadata repair owns its own version-scoped non-cancelling lock.
